...
#define NOTE_URL_PREFIX "hikki://note/"
...
static linkytype linkt  = { 0, 0, "<a href=\"", "\"",
                             0, ">", "</a>", DENY_A, IS_URL };
static linkytype interlinkt  = { 0, 0, "<a class=\"interlink\" href=\"", "\"",
                             0, ">", "</a>", DENY_A, IS_URL };
...
linkyformat(MMIOT *f, Cstring text, int image, Footnote *ref)
{
    ...
    if ( image )
	tag = &imaget;
    else if (ref->link.size >= sizeof("hikki://") - 1 &&
             !memcmp(ref->link.text, "hikki://", sizeof("hikki://") - 1))
      /* interlink */
      tag = &interlinkt;
		...
		if ( !S(key.tag) ) {
		    ...
		    S(key.tag) = S(name);
                ...
                if (name.size > 2 &&
                  ...
                  ref.title.size = 0;
                  ref.tag = name;
                  ref.link.size = linkname.size + sizeof(NOTE_URL_PREFIX) - 1;
                  ref.link.alloc = ref.link.size;
                  linkbuf = (char*) malloc(ref.link.size + 1);
                  strcpy(linkbuf, NOTE_URL_PREFIX);
                  strncpy(linkbuf + sizeof(NOTE_URL_PREFIX) - 1, linkname.text, linkname.size);
                  ref.link.text = linkbuf;
                  ref.dealloc = 0;
                  status = linkyformat(f, name, image, &ref);
                  free(linkbuf);

                  /* restore name text pointer so that it can be freed */
                  name.text--;
                }
                else if ( ref = bsearch(&key, T(*f->footnotes), S(*f->footnotes),
